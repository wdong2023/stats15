...
The graph of number of followers is unimodal and heavily skewed to the right. The mean and median number of followers is the same, which is around 49 people. Approximately 2.5% of values are below or equal to 0. However, the highest 2.5% are between 228 and 1045, which suggests that there are a few significantly popular houses. There are 2021 houses with 0 follower and also a total of 5 houses with more than 1000 followers. Among houses with more than 1000 followers, 1 house has significant low total price compare with others. However, it also has relative small area and is located in the suburb, which makes this reasonable. 
...
The graph of total price is diamond-shaped, unimodal, and skews right. The average total price of a house in Beijing in 2017 is 456x10,000 yuan (around 650~760 thousands dollars depend on inflation) and the median is 418x10,000 yuan (around 600~640 thousands dollars). 
...
There is one house has 0 bedroom but sold in 145,612 yuan per square meter which almost reached the highest square per meter sold among all observations (150,000 yuan). It is located in XiCheng. According to Lianjia website, this house belongs to "RongFeng 2008" community. After a research on the community, we found out that this is the average price per square meter for this community. An investigation conducted by The Beijing News indicates that "RongFeng 2008" has a relative 10,000 yuan higher price per square meter compared with other communities in that area. But since the houses in this community have very small areas, the total price is still acceptable for most people who wanted to move to XiCheng. 
...
Among these 188 houses that were sold within 1 day, 140 have no follower and the maximum number of followers is 25, which is not a significant number of followers. 
...
Haidian has the top 2 highest total prices. 
FangShan has the smallest range of total prices.
...
# are these 2 houses from the same community?
```



### 3.2.12 month (Categorical) vs. price

The total price over month seems to be proportional to the amount of house deals over month that March is the peak, followed by February and January, and May is the bottom.

```{r}
ggplot(data2017,aes(x=month,y=totalPrice))+
  geom_col(fill = "light blue") + 
  scale_x_discrete(limits = month.name)
```


### 3.2.13 price/totalPrice vs. floorRange

The graph of floor range over price and floor range over total price have the same trend. As shown under the 3.1.4 Floor Range variable, Medium floor range is the most popular one and has the highest price per square meter/total price. 

```{r}
ggplot(data2017, aes(floorRange,y=price))+
  geom_col(fill="lightblue") 

ggplot(data2017, aes(floorRange,y=totalPrice))+
  geom_col(fill="lightblue") 
```
