...
Now, the plot is unimodal and skews to the right.
...
The floor range did not make a huge influence on the price of the house. However, the price drops when the total floor is above 30, with one exception, which is the property that has 63 floors. In general, the price per square meter is slightly lower for properties located in buildings higher than 30 floors. 
...
The distribution is bimodal that the first peak of mean price is for houses with 1 bathroom and the second peak is for houses with 5 bathrooms. Price slightly changes as the number of bathrooms increase from 2 to 4. There is a sudden drop on price for houses with 6 bathrooms. It has mean price below 30,000 yuan, which is nealy 50% lower than the second lowest price, which is houses with no bathroom.
...
Let's compare how number of bathrooms affect price within each district. For most districts, houses with 0 or 1 bathroom have the highest price, except for ShiJingShan that does not have houses with no bathroom and the mean price increases as the number of bathroom increase. And for FengTai, the peak is at 3 and DaXing has the peak at 3.
```{r}
...
### 3.2.25 Price vs Days on Market
```{r}
...
### 3.2.26 Price vs Ladder Ratio vs District
```{r}
