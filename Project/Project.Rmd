...
ggplot(price_district_desc, aes(x=reorder(district, meanPriceDistrict), y=meanPriceDistrict))+
  geom_col(fill="lightblue") +
...
There is no huge differences on mean price per square on each floor range. As shown under the 3.1.4 Floor Range variable, Medium floor range is the most popular one. Medium and bottom floor ranges seem to have relative higher price whereas top floor range has the lowest.

```{r}
floor <-data2017 %>% 
  group_by(floorRange) %>%
  summarise(meanPrice2=mean(price))
ggplot(floor, aes(x=floorRange, y=meanPrice2))+
  geom_col(fill="lightblue") 
```
