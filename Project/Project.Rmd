...
- construction time
```{r}
table(data2$constructionTime)

barplot(table(data2$constructionTime))
```
- renovation condition
``` {r}
table(data2$renovationCondition)

barplot(table(data2$renovationCondition))
```
- building structure
``` {r}
table(data2$buildingStructure)

barplot(table(data2$buildingStructure))
```
- ladder ratio
```{r}
table(data2$ladderRatio)

barplot(table(data2$ladderRatio))
```
- elevator
```{r}
table(data2$elevator)

barplot(table(data2$elevator))
```
- five years property
```{r}
table(data2$fiveYearsProperty)

barplot(table(data2$fiveYearsProperty))
```
- subway
```{r}
table(data2$subway)
...
- bathroom
```{r}
table(data2$bathRoom)

barplot(table(data2$bathRoom))
```

potential relationships that may exist in the data
- construction time, renovation condition
```{r}
table(data2$constructionTime, data2$renovationCondition, prob= T)
