...
I hide code for this section because the file doesn't knit with Chinese characters in the code. 
...
waffle(counts)
```
Apartments located in the middle of a building were traded the most in 2017. Bottom and top both have lowest percentage, while low and high have relatively middle percentage. 
...
The distribution of total price is unimodal and skews right. The average total price is 456x10,000 yuan (around 650~760k USD depend on inflation) and the median is 418x10,000 yuan (around 600~640k USD). 
...
The graph of number of squares is unimodal and skewed to the right. The mean is around 80 square meters. The median is 72.4 square meters. Approximately 2.5% of values are below 37.3 square meters. Approximately 2.5% of values are above 166.6 square meters. 

We looked at the top 3 houses with the most square meters. Two of them are located in ShunYi, the rural area, which makes it reasonable to have large houses. Another one is located in ChaoYang, one of the core districts, but it has 6 bedrooms. We thought this house is also reasonable. 
...
The distribution of number of livling rooms is unimodal and skews right. The mean and median number of living room is 1. Approximately 2.5% of values are above 2. The number of living rooms for all houses sold in 2017 is between 0 to 4. 79% of properties have 1 living room. Properties with 0 living room is reasonable because these apartments are usually called "studios", which is common in the city. We also consider 4 living rooms as reasonable because sometimes dining rooms are counted as living rooms in China. 
...
XiCheng and Haidian have similar amount of houses sold where XiCheng is a smaller district than Haidian. \newline
...
ggplot(data2017, aes(x=district))+
  geom_bar(fill='lightblue', color='black') +
  xlab("district")+ylab("Count")+theme(axis.text.x = element_text(angle = 45, vjust = 0.5))+
...
Both suburb and core districts contain 6 districts. Though suburb have much larger areas shown on the map, 28% of houses sold are located in the suburb where core districts have houses sold that made up 72% of the market.
...
There are two houses with more than 2 kitchens, which seem odd. Taking a closer look, it had less than 300 square meters, which is impossible to have 3 kitchens in fairly medium house size. Therefore, we removed these this outlier.
...
```
We took a closer look at properties that have more than 4 bathrooms. It's odd to have more bathrooms than bedrooms, so we removed the three  properties that had more bathrooms than bedrooms. 
...
The year of construction of the listings that were transacted in 2017 ranges from the year 1950 to the year 2016.

The median is 2002. Approximately 2.5% of values are above 2014. Approximately 2.5% of values are below 1980. 
...
The distribution for the condition of renovation shows that most listings have the highest rating of hardcover in terms of the condition of renovation for their listing and simplicity is the second, showing that most of the houses sold in 2017 are in good renovation condition. 
...
The distribution for the building structure shows that most listings are made of steel-concrete composite, with mixed being second. There are very few properties made of brick&wood, which is uncommon in a big city. There are also very few steel houses. 
...
The median is 0.333. Approximately 2.5% of values are above 0.667. Approximately 2.5% of values are below 0.121.
...
Now, the plot is unimodal and skews to the right, with most values ranging between 0 and 1. 
...
The distribution shows that among the previous owners of the listings, a higher number of them have stayed in the listing for less than five years and a lower number of them have stayed in the listing for exactly five years or more than five years.
...
Among these 474 houses that were sold within 1 day, 376 have no follower and the maximum number of followers is 39, which is not a significant number. Our guessing is that the owner and buyer might have already planned about this deal privately and they just list the property on an official third-party website like LianJia to make the transaction secure and supervised.
```{r}
...
cor(data2017$followers, data2017$price)
```

The above graph does show a slight negative trend between price and followers. As followers increase, the price of the perperty is lower, although this trend does not apply to houses with very few followers because many properties ranging from low price to high price had few followers. The relationship is very week with a correlation of -0.066.

### 3.2.3 Days on Market vs Price 
```{r}
...
There is a slight decreasing trend for price as days on market increases. The correlation is as small as -0.03 and the fitting line is almost flat, so we predict days on market does not have significant relationship with average house price. 


### 3.2.4 Month vs Price

Overall, there is no huge differences on mean price per square on each month. As shown under the month variable section that March has the most transactions, the price is also at the peak on March. April and May also have relative higher price. After March, the price starts to decrease and reaches the bottom at December. 
...
```{r}
march_price <- data2017 %>%
  filter(month== "March") %>%
  select(price) %>% pull()
dec_price <- data2017 %>%
  filter(month== "December") %>%
  select(price) %>% pull()
t.test(march_price, dec_price)
```

The result of t-test is consistent with the graph, showing that the average house price sold in March is significantly higher than average house price sold in December. 




### 3.2.5 District vs Price
...
Taking a look at boxplot, XiCheng still has the highest range of price and the second highest range is DongCheng. FangShan still got the lowest price range. The distribution of median prices is the same as the distribution of mean price. It seems like both HaiDian and XiCheng have big range of house price. But XiCheng has overall higher prices than ChaoYang.
...
### 3.2.6 District vs Total Price

ChaoYang has the top 2 highest total prices. FangShan has the smallest range of total prices. From the boxplot, we can see that DongCheng, HaiDian, and XiCheng are the 3 districts that have relative high total prices.
...
### 3.2.7 District vs Square

As indicated under the 'square' variable, the top 2.5% of houses sold have area over 166.6 square meters. Let's define them as "large houses". The bottom 2.5% of houses sold have less than 37.27 square meters. Let's define them as "small houses".

*Districts with most large and small houses*
ChaoYang and ChangPing has the significant highest amounts of large houses sold and XiCheng has the significant highest amounts of small houses sold. 
...
ggplot(pivot,aes(x=reorder(district, value),y=value,fill=name))+
  ...
  facet_wrap(~name)+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
  labs(title = "Small and Large Souses in Districts", x = "district")
```

*Proportion of large and small houses within each district*
The majority of houses for all districts are middle sized. ChangPing and ChaoYang has the highest proportion of large houses followed by ShunYi which has the largest proportion of small houses. MenTouGou did not have extreme large and small houses. 
```{r}
middle <- data2017 %>%
...
### 3.2.8 Square vs Price

```{r}
ggplot(data2017, aes(x=square, y= price)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~district)
```

Even though the price is per square meter, it still has a relationship with square when we plot them by district. For most districts, price per square meter decreases as square increases, except for ChaoYang and ShiJingShan. Properties in medium sizes are more expensive probably because the demand is higher. 




### 3.2.9 Kitchen vs Price

Houses with more kitchens have higher mean prices. The difference between 0 and 1 kitchen is about 10,000 yuan, but the difference between 1 and 2 kitchens is not significant. Let's compare how the number of kitchens affect prices within each district.
...
For most districts, houses with 1 kitchen have the highest mean price. Except for ChangPing, ChaoYang, and FengTai, houses with more kitchens have higher mean prices. HaiDian is the special case that the mean house prices keep decreasing as the number of kitchens increase.
...
### 3.2.10 Bedroom vs Price

The more bedrooms indicates the lower mean house price is. However, simply look at the mean price for all houses is not enough because district plays the key role. Let's compare how the number of bedrooms affect price within each district.
...
There is also a trend observed for some districts that the more bedrooms the lower price is. ChangPing is an exception, where lots of huge maisons are located. It could be popular in ChangPing to have many bedrooms in a house. HaiDian is another exception where price increases for 6 bedroom, which could be due to that it's rare to see such large houses in HaiDian because it's one of the core districts. 
...
### 3.2.11 Living room vs Price

The graph shows that there is a decreasing trend for price as number of bedrooms increase. Houses sold with no living room have the highest mean price. As shown under the living room variable section, most houses have 1 living room. The price of houses with 1 living room is slightly lower. However, the mean price slightly increases for 4-bedrooms. Let's compare how the number of living rooms affect house price within each district.
...
For most district, houses with 0 or 1 living room still have relative higher price. Except for ShiJingShan, which has higher mean price for houses with 4 bedrooms, mainly because ShiJingShan is the smallest district in Beijing. It's more expensive to buy big houses in ShiJingShan. 
...
### 3.2.12 Bathroom vs Price

```{r}
table(data2017$bathRoom==0)
```

Houses with 1 bathroom have the highest mean price and houses with 5 bathrooms have the second highest price. Price slightly changes as the number of bathrooms increase from 2 to 4. There is a sudden drop on price for houses with 6 bathrooms. It has mean price below 30,000 yuan, which is nealy 50% lower than the second lowest price, which is houses with no bathroom.
...
Let's compare how number of bathrooms affect price within each district. For most districts, houses with 1 or 5 bathroom have the highest price. 
HaiDian is the special case that houses with either no bathroom or the maximum number of bathrooms have the highest prices. For ShiJingShan and TongZhou, more bathrooms indicate higher prices.
...
### 3.2.13 Ladder Ratio and Floor Range vs Price
```{r}
...
It's hard to see any relationships between these three from this graph. However, the price drops when the total floor is above 30, with one exception, which is the property that has 63 floors. In general, the price per square meter is slightly lower for properties located in buildings higher than 30 floors. Next, lets' take a look at them with price separately. 



### 3.2.14 Floor Range vs Price

There is a slight decreasing trend as properties are located higher. The higher the property is located, the lower the average price is. Medium and bottom floor ranges seem to have relative higher price whereas top floor range has the lowest.
...
### 3.2.15 Ladder Ratio vs District vs Price
```{r}
ggplot(data2017, aes(x = ladderRatio, y = price)) +
  geom_point(color = "blue") +
  facet_wrap(~district)+
  labs(x = "Ladder Ratio", y = "Price", title = "Price vs Ladder Ratio and Districts") +
  geom_smooth(color = "black")
cor(data2017$price, data2017$ladderRatio)
```

There isn't a significant relationship between ladder ratio and price. The correlation is only -0.13. Therefore, we predict that the ladder ratio will not significantly influence price. \newline
Most districts have ladder ratio between 0 to 1, while ChaoYang has the most properties with higher than 1 ladder ratio. 


### 3.2.16 Elevator vs District vs Price

From the distribution, we can see that most of the listings are more expensive when there is an elevator available in the building with the exception of HaiDian, DongCheng, XiCheng.

``` {r}
data2017 %>% group_by(district, elevator) %>%
  summarise(mean_price = mean(price)) %>%
  ggplot(aes(x = reorder(district, mean_price), y = mean_price, fill = elevator)) +
...
### 3.2.17 Subway vs District vs Price
...
Let's see how a house is near subway or not will affect the price within each district. The conclusion is that for most districts, mean price is higher if the house is near subway. However, DaXing is an exception where mean price seems to be lower for properties that are near a subway. 


### 3.2.18 Construction time vs District

From the distribution of time of construction by district, we see that for the district of ChaoYang, there is a gradual increase in the number of listings being constructed since 1950 and then a sharp decrease after it reached its peak. \newline
For the district of ChangPing, there is a sharper increase in the number of listings being constructed since 1950 compared to ChaoYang but a more gradual decrease after it reached its peak. \newline
For the other districts, the number of listings being constructed changes less drastically compared to ChaoYang and ChangPing.
...
### 3.2.19 Construction time vs Elevator vs Price


``` {r}
data2017 %>% group_by(constructionTime, elevator) %>%
  summarise(mean_price = mean(price)) %>%
  ggplot(aes(x = constructionTime, y = mean_price, color = elevator)) +
  geom_line() +
  ggtitle("Mean Price Change with Construction Time and Presence of Elevator") +
  labs(x = "Time of Construction", y = "Mean Price") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"), 
        axis.title.x = element_text(face="bold", size = 12),
        axis.title.y = element_text(face="bold", size = 12),
        legend.title = element_text(face="bold", size = 10))
```
From the graph, we can see that as properties are constructed in later years, the mean price drops. In othe words, the newer the property is, the lower the average price. For some reason, houses construced during 1970-1980 are exceptionally expensive. The presence of elevator emerged for properties built after 1970 in Beijing. Consistent with previous findings, mean price is higher for properties with elevators compared to those without elevator that are constructed in the same year. 


### 3.2.20 Renovation Condition vs District

From the distribution of renovation condition by district, we see that ChaoYang has the highest number of listings with the *hardcover* rating. ChaoYang had a lot of well-renovated properties being sold compared to other districts because ChaoYang is one of the core financial districts where many wealthy poeple live. 
...
### 3.2.21 Renovation Condition vs District vs Price

For most districts, hardcover renovation has higher mean price than rough renovation condition with the exception of TongZhou, MenTouGou, and Haidian. 
...
### 3.2.22 Elevator vs. Building structure
...
### 3.2.23 Building structure vs Price
...
### 3.2.24 Five years property vs District
...
### 3.2.25 Five years property vs District vs Price

From the distribution, we see that most of the listings are more expensive when the previous owner has stayed in the listing for more than five years except for  ShiJingShan, FengTai and DongCheng.
...
```
In this model, I randomly split data into training set and testing set, which is the validation set approach. Based on the results of linear model, all the variables are important except for five year property, the day in a month being sold, and building structure. 

### Coefficient interpretation:
- if days on market increase by 1, the average price increase by 12.18
- if followers increase by 1, the average price decreases by 41.1
- if square increases by 1, the average price decreases by 180
- if number of bedroom increase by 1, the average price increases by 1312
- if number of living room increase by 1, the average price increases by 1682
- if number of bkitchen increase by 1, the average price increases by 10356
- if number of bathroom increase by 1, the average price increases by 5761
- if construction year increase by 1, the average price decreases by 136.8
- if ladder ratio increase by 1, the average price increases by 12759
- if total number of floors increase by 1, the average price decreases by 116
- if the house is sold in April, we expect the price to be higher than houses sold in other months. 
- if there is a subway near the house, we would expect the price to be 5116 Yuan higher than houses without a subway.
- if there is a elevator near the house, we would expect the price to be 5500 Yuan higher than houses without a elevator
- Being in ChaoYang, DongCheng, FengTai, HaiDian, ShiJingShan, TongZhou, and XiCheng increases the price with a significant amount compared to ChangPing because these are all core districts that are located in the center of Beijing.
- Being in FangShan, MenTouGou and ShunYi significantly decreases the price compared to ChangPing because these are the rural districts. 
- The price in DaXing is not significantly different from price in ChangPing since these two are both rural districts. 
- If the apartment is at the top, we would expect the price to be 2354 Yuan lower than houses at the bottom. 
...
# training MSE
pred_train_lm <- predict(backward_model, train, type="response")
mean((pred_train_lm-train$price)^2)
# testing MSE
pred_test_lm <- predict(backward_model, test, type="response")
mean((pred_test_lm-test$price)^2)
```

The resulting MSE from backward regression is larger than the MSE from simple linear regression that uses all the predictors, so we prefer the one with all predictors. 
...
## 4.6 Fitting Model For Core Districts
```{r}
core_district <- data2017 %>%
  filter(district %in% c("XiCheng" , "DongCheng" , "ChaoYang" , "HaiDian" , "ShiJingShan" , "FengTai"))

set.seed(428)
index <- sample(nrow(core_district),15123,replace = FALSE)
train <- core_district[index,c(2,3,5:22)] # exclude total price 
test <- core_district[-index,c(2,3,5:22)] # exclude total price

# training MSE for linear model
pred_train_lm <- predict(linear_model, train, type="response")
mean((pred_train_lm-train$price)^2)
# testing MSE for linear model
pred_test_lm <- predict(linear_model, test, type="response")
mean((pred_test_lm-test$price)^2)
...
pred_train_rf <- predict(rf_model, train, type="response")
mean((pred_train_rf-train$price)^2)
# testing MSE for random forest
pred_test_rf <- predict(rf_model, test, type="response")
mean((pred_test_rf-test$price)^2)

```
Only fitting core districts in the linear model and random forest model does not improve MSE, and even generates larger MSE.


## 4.7 Fitting Model for Rural Districts
```{r}
suburb <- data2017 %>%
  filter(district %in% c("DaXing", "ChangPing", "FangShan", "MenTouGou", "ShunYi", "TongZhou"))

set.seed(428)
index <- sample(nrow(suburb),5878,replace = FALSE)
train <- suburb[index,c(2,3,5:22)] # exclude total price 
test <- suburb[-index,c(2,3,5:22)] # exclude total price
...
```
Fitting rural districts in the linear model significantly improves MSE, but not for random forest. Therefore, the linear model better predicts house prices in rural districts, while random forest better predicts house prices in all districts together. 


# 5 Conclusion
We found that people like to trade houses in the beginning of the year for 2017. Apartments with 2 bedrooms, 1 living room, 1 kitchen, 1 bathroom, and well renovated with elevator and subway near by was the most popular type of housing for transaction. \newline
The most important predictors for price based on the Random Forest model and Backward Regression model are district, subway, square, and construction time, which is consistent with the relationships we investigated in section 3. \newline
As we predicted, the core districts generally have higher price per square meter compared to rural districts. There is no denying that district is the most essential factor that affects the price. Thus, we need to consider districts while comparing other variables. Among those, ChaoYang is "special". It is a core district but has great amounts of transactions and also significant high amounts of houses with big square meters. The total prices have a wide range as well. That is, you could find almost every kind of house in ChaoYang. \newline
The availability of a subway near by is associated with higher average price. The price also decreases as square meters goes up. As for construction time, the later the property was constructed, the lower the average price is. We were expecting "sold fast" and "have many followers" to indicate the popularity of the property. But the fact turns out to be there are actually many houses sold within a very short period of time that have no followers and there is actually not much trend observed between these 2 variables.\newline
Despite *hardcover* representing the best renovation condition, *other* represents the worst renovation condition yet people are willing to pay a similar price to *hardcover*. Listings that have been previously occupied for more than five years are associated with higher prices.



### **Work Cited**
some online resources that we used
