...
# I don't know what you are trying to show here, but if you graph the average price vs month (code below), you will see that the house price does not vary depend on month. In other words, month does not have a relationship with price, which is what we want to know from this section. 

data2017%>%
  group_by(month) %>%
  summarise(avg_price = mean(price)) %>%
  ggplot(aes(x=month, y=avg_price)) + geom_col(fill="light blue")
...
The graph of floor range over price and floor range over total price have the same trend. As shown under the 3.1.4 Floor Range variable, Medium floor range is the most popular one and has the highest price per square meter/total price. There is no significant difference between other fllor ranges.

```{r}
# please change this section as well, same thing as month
